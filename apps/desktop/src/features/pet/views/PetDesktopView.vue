...
import { buildInteractionBubbles } from "@/features/pet/services/petDialogues";
import type { PetEventLedgerEntry, PetInteractionAction, PetWorkflowEventType } from "@/shared/types/meeting";
...
const PET_STATE_REFRESH_INTERVAL_MS = 15000;
...
function getEventBubble(event?: PetEventLedgerEntry) {
  ...
  const eventBubbles = EVENT_BUBBLES as Record<PetWorkflowEventType | "daily_open", string[]>;
  const candidates = (interactionBubbles.value as Record<string, string[]>)[key] ?? eventBubbles[key as keyof typeof eventBubbles];
...
const environmentState = computed(() => getPetEnvironmentState(meetingStore.jobs.value));
...
async function refreshPetWindowState() {
  try {
    ...
    if (petStore.settings.value && !petStore.settings.value.desktopEnabled) {
