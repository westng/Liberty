...
import type { PetSettings } from "@/types/meeting";
...
async function resolvePetWindowPlacement(
  ...
  try {
    ...
    const isVisibleOnAnyMonitor = monitors.some((monitor) => {
      ...
      return (
        settings.lastWindowX >= position.x
        && settings.lastWindowY >= position.y
        && settings.lastWindowX <= maxX
        && settings.lastWindowY <= maxY
      );
    });
